...
class App(tk.Tk):
    def __init__(self):
        ...
        register("commandes_client", CommandeClientTab)
...
class PaieBulletinsTab(ttk.Frame):
    ...
    def _calculer_retenues(self):
        ...
        resultat = core.compute_bulletin_paie(bulletin, params)
    ...
    def imprimer_bulletin(self):
        if not self.selected_bulletin_id:
            ...
            return
        html = core.render_bulletin_paie_html(self.conn, self.selected_bulletin_id)
    ...
    def save_bulletin(self):
        ...
        try:
            ...
            messagebox.showerror("Erreur", str(exc))
            return
        self.selected_bulletin_id = core.get_bulletin_paie(self.conn, p["id"], periode)["id"]
        self.refresh()
        self._calculer_retenues()
...
class TresorerieTab(ttk.Frame):
    ...
    def _build_engagements(self, parent):
        ...
        cols = ("numero", "date", "fournisseur", "montant", "statut")
        self.tree_engagements = ttk.Treeview(parent, columns=cols, show="headings", height=14)
        headers = ["N° Règlement", "Date", "Fournisseur", "Montant restant à payer", "Statut"]
        widths = [130, 100, 240, 170, 220]
        for c, h, w in zip(cols, headers, widths):
            self.tree_engagements.heading(c, text=h)
            self.tree_engagements.column(c, width=w, anchor="w" if c not in ("montant",) else "e")
    ...
    def _build_echeances(self, parent):
        ttk.Label(parent, text=(
            ...
            "par mois avec le(s) compte(s) de charge concerné(s) — pour anticiper l'impact sur la "
            "trésorerie plusieurs mois à l'avance."
        ), foreground="#595959", wraplength=1050).pack(anchor="w", padx=8, pady=(8, 8))
        ...
        ttk.Label(top, text="Nombre de mois :").pack(side="left", padx=(12, 4))
        self.echeance_nb_mois_var = tk.StringVar(value="6")
        ttk.Entry(top, textvariable=self.echeance_nb_mois_var, width=4).pack(side="left")
        ttk.Button(top, text="Actualiser", command=self.refresh).pack(side="left", padx=8)
    ...
    def refresh(self):
        ...
        try:
            ...
            nb_mois = 6
        p = core.compute_echeances_tresorerie_pivot(self.conn, date_from=date_from, nb_mois=nb_mois)
...
class CommandeClientEcheancierDialog(tk.Toplevel):
    """Planifie l'échéancier de règlement PRÉVU d'une Commande client —
    une ou plusieurs tranches, avant même la génération de la facture.
    Sera automatiquement reprise sur la facture générée à la validation."""

    def __init__(self, parent, conn, commande_id, montant_ttc, on_saved):
        super().__init__(parent)
        self.conn = conn
        ...
        self.tranches = [{"date_echeance": t["date_echeance"], "montant": t["montant"]}
                          for t in core.list_echeances_commande_client(conn, commande_id)]
        self._refresh_tree()
    ...
    def enregistrer(self):
        if not self.tranches:
            ...
            return
        try:
            core.set_echeancier_commande_client(self.conn, self.commande_id, self.tranches)
        except ValueError as exc:
            messagebox.showerror("Erreur", str(exc), parent=self)
            return
...
class CommandeClientDialog(tk.Toplevel):
    """Détail d'une Commande client (clic depuis la liste). La validation
    fait basculer la commande en FACTURE DE VENTE (brouillon, non
    comptabilisée) avec les lignes recopiées — voir core.valider_commande_client()."""

    def __init__(self, parent, conn, commande_id, on_saved):
        super().__init__(parent)
        self.conn = conn
        self.commande_id = commande_id
        self.on_saved = on_saved
        ...
        commande = core.get_commande_client(conn, commande_id)
    ...
    def _refresh_ligne_compte_values(self):
        items = [a for a in core.search_accounts(self.conn, "7", limit=100) if a["classe"] == "7"]
    ...
    def refresh_lignes(self):
        for row in self.tree.get_children():
            self.tree.delete(row)
        total = 0.0
        for l in core.list_lignes_commande_client(self.conn, self.commande_id):
            total += l["montant_ht"]
            self.tree.insert("", "end", values=(
    ...
    def add_ligne(self):
        ...
        core.add_ligne_commande_client(self.conn, self.commande_id, libelle, qte, prix_unitaire=prix,
    ...
    def update_ligne(self):
        ...
        compte = self._extract_code(self.ligne_compte_var.get()) or None
        analytic = self._extract_code(self.ligne_analytic_var.get()) or None
        core.update_ligne_commande_client(
            self.conn, self.selected_ligne_id, libelle=libelle, quantite=qte, prix_unitaire=prix,
            unite=self.ligne_unite_var.get().strip() or None, compte_vente=compte, analytic_code=analytic)
        self._clear_ligne_form()
        self.refresh_lignes()
    ...
    def delete_ligne(self):
        if not self.selected_ligne_id:
            messagebox.showinfo("Info", "Sélectionnez d'abord une ligne.", parent=self)
            return
        core.delete_ligne_commande_client(self.conn, self.selected_ligne_id)
        self._clear_ligne_form()
        self.refresh_lignes()

    def _refresh_retard(self):
        commandes = core.list_commandes_client(self.conn)
    ...
    def _refresh_echeancier_apercu(self):
        tranches = core.list_echeances_commande_client(self.conn, self.commande_id)
        if not tranches:
    ...
    def modifier_echeancier(self):
        totals = core.compute_commande_client_totals(self.conn, self.commande_id)
        if totals["total_ttc"] <= 0:
            ...
            return
        CommandeClientEcheancierDialog(self, self.conn, self.commande_id, totals["total_ttc"],
                                        self._refresh_echeancier_apercu)
    ...
    def save(self):
        ...
        try:
            ...
            return
        core.update_commande_client(
            self.conn, self.commande_id,
        ...
        self._refresh_retard()
        messagebox.showinfo("Enregistré", "Commande enregistrée.", parent=self)
        self.on_saved()
    ...
    def valider(self):
        ...
        self.save()
        try:
            core.valider_commande_client(self.conn, self.commande_id)
        except ValueError as exc:
            messagebox.showerror("Erreur", str(exc), parent=self)
            return
        messagebox.showinfo(
    ...
    def corriger(self):
        if not messagebox.askyesno(
            ...
            return
        try:
            core.devalider_commande_client(self.conn, self.commande_id)
        except ValueError as exc:
            messagebox.showerror("Erreur", str(exc), parent=self)
            return
        self.validee = False
...
class CommandeClientTab(ttk.Frame):
    """Liste des Commandes client (menu COMMERCIAL) — clic pour ouvrir et
    valider (bascule en Facture de vente). Miroir exact du circuit Bon de
    commande fournisseur, côté ventes : comptes de produits et clients."""
    ...
    def new_commande(self):
        ...
        if not numero:
            return
        cid = core.create_commande_client(self.conn, numero, date.today().strftime("%Y-%m-%d"))
        self.refresh()
        CommandeClientDialog(self, self.conn, cid, on_saved=self.refresh)
    ...
    def refresh(self):
        for row in self.tree.get_children():
            self.tree.delete(row)
        self._by_iid = {}
        for commande in core.list_commandes_client(self.conn):
            nb = len(core.list_lignes_commande_client(self.conn, commande["id"]))
            statut = "Validée" if commande["statut"] == "validee" else "Brouillon"
            client = ""
            if commande["client_code"]:
                c = core.get_client(self.conn, commande["client_code"])
                client = f"{commande['client_code']} — {c['raison_sociale']}" if c else commande["client_code"]
            tags = ("retard",) if commande.get("depassement_paiement") else ()
            iid = self.tree.insert("", "end", tags=tags, values=(
                commande["numero"], core.to_display_date(commande["date_commande"]), client, nb, statut,
...
class FactureEcheancierDialog(tk.Toplevel):
    ...
    def __init__(self, parent, conn, facture_id, net_a_payer, on_saved):
        super().__init__(parent)
        self.conn = conn
        self.facture_id = facture_id
        ...
        self.tranches = [{"date_echeance": t["date_echeance"], "montant": t["montant"]}
                          for t in core.list_echeances_facture_achat(conn, facture_id)]
        self._refresh_tree()
    ...
    def enregistrer(self):
        if not self.tranches:
            ...
            return
        try:
            core.set_echeancier_facture_achat(self.conn, self.facture_id, self.tranches)
        except ValueError as exc:
            messagebox.showerror("Erreur", str(exc), parent=self)
            return
        messagebox.showinfo("Enregistré", "Échéancier planifié.", parent=self)
...
class FacturesFrsTab(ttk.Frame):
    ...
    def __init__(self, parent, conn):
        ...
        echeancier_frame = ttk.Frame(info)
        echeancier_frame.grid(row=4, column=0, columnspan=6, sticky="we", padx=4, pady=(6, 0))
        ttk.Button(echeancier_frame, text="Planifier un échéancier (plusieurs tranches, optionnel)",
                   command=self.modifier_echeancier_facture).pack(side="left")
        self.echeancier_apercu_var = tk.StringVar()
        ttk.Label(echeancier_frame, textvariable=self.echeancier_apercu_var, foreground="#595959").pack(
    ...
    def _refresh_echeancier_apercu(self):
        if not self.current_facture_id:
            self.echeancier_apercu_var.set("")
            return
        tranches = core.list_echeances_facture_achat(self.conn, self.current_facture_id)
        if not tranches:
            self.echeancier_apercu_var.set("Aucun échéancier planifié (paiement en une fois à la date ci-dessus).")
        else:
            self.echeancier_apercu_var.set(
    ...
    def modifier_echeancier_facture(self):
        ...
        if totals["net_a_payer"] <= 0:
            messagebox.showinfo("Info", "Ajoutez d'abord des lignes avec un montant avant de planifier "
                                         "un échéancier.")
            return
        FactureEcheancierDialog(self, self.conn, self.current_facture_id, totals["net_a_payer"],
                                 self._refresh_echeancier_apercu)
...
class EcheancierDialog(tk.Toplevel):
    """Définit l'échéancier de paiement d'un Règlement — une ou plusieurs
    tranches, chacune avec sa propre date, pour un engagement payable en
    plusieurs fois sur plusieurs mois plutôt qu'en un seul versement."""

    def __init__(self, parent, conn, reglement_id, net_a_payer, on_saved):
        super().__init__(parent)
        self.conn = conn
        ...
        self.tranches = []
        existantes = core.list_echeances_reglement(conn, reglement_id)
        for e in existantes:
            if e["statut"] != "Payée":
                self.tranches.append({"date_echeance": e["date_echeance"], "montant": e["montant"]})
        self._refresh_tree()
    ...
    def enregistrer(self):
        if not self.tranches:
            ...
            return
        try:
            core.set_echeancier_reglement(self.conn, self.reglement_id, self.tranches)
        except ValueError as exc:
            messagebox.showerror("Erreur", str(exc), parent=self)
            return
        messagebox.showinfo("Enregistré", "Échéancier mis à jour.", parent=self)
...
class ReglementDialog(tk.Toplevel):
    ...
    def __init__(self, parent, conn, reglement_id, on_saved):
        ...
        echeancier_frame = ttk.LabelFrame(header, text="Échéancier de paiement (une ou plusieurs tranches)")
        echeancier_frame.grid(row=2, column=0, columnspan=6, sticky="we", padx=4, pady=(8, 0))
        ttk.Button(echeancier_frame, text="Modifier l'échéancier (plusieurs tranches possibles)",
                   command=self.modifier_echeancier).grid(row=0, column=0, padx=4, pady=4, sticky="w")
        ...
        for c, h, w in zip(cols_ech, ["ID", "Tranche", "Échéance", "Montant", "Statut"], [0, 60, 100, 120, 100]):
            self.tree_echeances.heading(c, text=h)
        ...
        self.tree_echeances.grid(row=1, column=0, columnspan=6, sticky="we", padx=4, pady=4)
        self.tree_echeances.tag_configure("retard", foreground="#B00020")
        self.tree_echeances.tag_configure("payee", foreground="#1F7A1F")

        paiement_frame = ttk.Frame(echeancier_frame)
        paiement_frame.grid(row=2, column=0, columnspan=6, sticky="we", padx=4, pady=(0, 6))
        ttk.Label(paiement_frame, text="Payer la tranche sélectionnée — Date :").pack(side="left")
        self.date_paiement_var = tk.StringVar(value=date.today().strftime("%d/%m/%Y"))
        ttk.Entry(paiement_frame, textvariable=self.date_paiement_var, width=12).pack(side="left", padx=4)
        ttk.Label(paiement_frame, text="Compte banque/caisse :").pack(side="left", padx=(12, 4))
        self.compte_paiement_var = tk.StringVar()
        self.compte_paiement_combo = ttk.Combobox(paiement_frame, textvariable=self.compte_paiement_var, width=26)
        self.compte_paiement_combo.pack(side="left", padx=4)
        self.compte_paiement_combo.bind("<KeyRelease>", self._on_compte_paiement_keyrelease)
        self.compte_paiement_combo.bind("<Button-1>", self._open_dropdown)
        self._refresh_compte_paiement_values()
        self.paiement_btn = ttk.Button(paiement_frame, text="Enregistrer le paiement de cette tranche",
                                        command=self.enregistrer_paiement)
        self.paiement_btn.pack(side="left", padx=12)
        self.paiement_statut_var = tk.StringVar()
        ttk.Label(header, textvariable=self.paiement_statut_var, foreground="#1F7A1F").grid(
            row=3, column=0, columnspan=6, sticky="w", padx=4, pady=(2, 0))
    ...
    def refresh_echeances(self):
        for row in self.tree_echeances.get_children():
            self.tree_echeances.delete(row)
        if not self.validee:
            self.paiement_statut_var.set("Validez d'abord le règlement pour voir son échéancier.")
            return
        echeances = core.list_echeances_reglement(self.conn, self.reglement_id)
        for e in echeances:
            tag = "payee" if e["statut"] == "Payée" else ("retard" if e["statut"] == "En retard" else "")
        ...
        if restant <= 0 and echeances:
            ...
            self.paiement_statut_var.set(f"Reste à payer : {fmt_cfa(restant)} F CFA.")

    def modifier_echeancier(self):
        if not self.validee:
            messagebox.showinfo("Info", "Validez d'abord le règlement avant de définir son échéancier.",
                                 parent=self)
            return
        totals = core.compute_reglement_totals(self.conn, self.reglement_id)
        EcheancierDialog(self, self.conn, self.reglement_id, totals["net_a_payer"], self.refresh_echeances)
    ...
    def enregistrer_paiement(self):
        ...
        if not compte:
            ...
            return
        try:
            montant = core.enregistrer_paiement_echeance(self.conn, echeance_id, date_paiement, compte)
        except ValueError as exc:
            messagebox.showerror("Erreur", str(exc), parent=self)
            return
        self.refresh_echeances()
